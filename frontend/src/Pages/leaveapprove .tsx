...
const LeaveApprove=()=>{
    ...
    const approvehandler=(_id:string)=>{
      console.log(_id);
      axios.post('http://localhost:3002/leaveapproval',{_id:_id})
      .then(responce=>{
        console.log(responce.data);
    ...
    const rejecthandler=(_id:string)=>{
      axios.post('http://localhost:3002/rejecthandler',{_id:_id})
      .then(responce=>{
        console.log(responce.data);
        message.success(responce.data.message)
          ...
          <div className="tw-py-2 tw-inline-block tw-min-w-full sm:tw-px-6 lg:tw-px-8">
            <div className="tw-overflow-hidden">
              <table className="tw-min-w-full">
                ...
                <tbody>
                  {studentlist && studentlist.map((list :any,index :any) => {
                    return (
                      <tr className="tw-bg-white tw-border-b tw-transition tw-duration-300 tw-ease-in-out hover:tw-bg-gray-100" key={list._id}>
                        ...
                        <td className="tw-text-sm tw-text-gray-900 tw-font-light tw-px-6 tw-py-4 tw-whitespace-nowrap ">
                          ...
                          <button className='tw-bg-blue-500 tw-text-white tw-p-3 tw-rounded-lg' onClick={()=>{approvehandler(list._id)}}> Approve</button>
                          <button className='tw-bg-red-500 tw-text-white tw-p-3 tw-rounded-lg ' onClick={()=>{rejecthandler(list._id)}}>Reject</button>
                          </div>
                        </td>
                      </tr>
                    );
